...
$stmt = $pdo->query('SELECT id, judul, kategori, file_path, file_type, ukuran, versi, created_at FROM arsips ORDER BY created_at DESC LIMIT 50');
$rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
if (! $rows) {
    echo "No rows found in arsips table.\n";
    exit(0);
}
foreach ($rows as $row) {
